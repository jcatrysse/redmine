...
| `tools/check-patch-clean.sh <slug>` | G6 — controleert het **patchbestand** en vergelijkt het met de branch; `--submit` maakt "applyt niet meer op trunk" fataal — maar meet tegen de **mirror** `origin/master`, dus laat Jan die eerst syncen en controleer het verschil zelf (K-19 optie A, commando's in `docs/runbook.md`). Sinds K-16 leest hij ook de auteur- en committervelden van de eigen commits van de branch, want `git format-patch` zet de committer niet in het bestand en een force-push komt niet langs `session-push.sh` |

...
| `tools/session-push.sh [branch]` | pushen als er parallelle sessies zijn: fetch, jouw commits erbovenop, vier keer opnieuw proberen. Gebruik dit in plaats van `git push`, ook op `7.0-stable-GEOxyz`. Sinds K-13 behoudt de replay de committer, en weigert hij een push naar `7.0-stable-GEOxyz` of `patch/*` met een AI-identiteit in het auteur- of committerveld. Sinds ronde 4 draait hij op `geoxyz/framework` ook `check-ownership.sh --infer`, en gebruikt hij één gedeeld INV-4-patroon uit `tools/inv4-identity.sh` — het zijne was het smalste van de drie en liet Cursor, Copilot en Codex door. Kan de range niet opgelost worden, dan weigert hij in plaats van schoon te melden. Framework-sessie? `FRAMEWORK_CHANGE=1` |
...
| `tools/check-ownership.sh <slug>` \| `--infer` | weigert een push die bestanden van iemand anders aanraakt; `--infer` leidt de slug af uit de gewijzigde paden en is wat `session-push.sh` draait. De twee **gegenereerde** bestanden (`docs/REGISTER.md`, `docs/review/FINDINGS.md`) mag je wél meesturen, maar ze moeten vers gegenereerd zijn — een met de hand bewerkt gegenereerd bestand faalt |
| `tools/check-patch-clean.sh <slug>` | G6 — controleert het **patchbestand** en vergelijkt het met de branch; `--submit` maakt "applyt niet meer op trunk" fataal, en haalt sinds K-20 zelf `redmine.org` op om te weigeren als de mirror `origin/master` achterloopt. Syncen blijft Jans werk (K-19 optie A, commando's in `docs/runbook.md`); de gate zegt alleen niet langer ja op een vraag die ze niet kon stellen. Sinds K-16 leest hij ook de auteur- en committervelden van de eigen commits van de branch, want `git format-patch` zet de committer niet in het bestand en een force-push komt niet langs `session-push.sh` |
| `tools/check-geoxyz-branch.sh` | G8 — lint gemeten tegen de baseline op `origin/7.0-stable`, alleen wat de branch toevoegt telt. **`INCOMPLETE` is geen `PASS`**: ontbreekt RuboCop of komt er geen leesbare JSON uit, dan staat er `????` en is de uitkomst niet-nul. Voor Redmine heb je naast `rubocop` ook `rubocop-performance` en `rubocop-rails` nodig, anders start hij niet eens |
| `tools/check-symmetry.sh <slug>` \| `--all` \| `--self-test` | INV-10 mechanisch, in twee vergelijkingen: **code** per regel (staat elke inhoudelijke regel van `patch/<slug>` ook op `7.0-stable-GEOxyz`?) en **locales per sleutel, op waarde**, met een echte YAML-parser. `check-geoxyz-branch.sh` ziet dit niet — die keurt de branch, niet de gelijkheid per feature. Let op wat de locale-helft betekent: geen andere gate leest ooit een vertaalde *waarde*, ze filteren alleen welke locale-bestánden mogen meedoen (INV-5). Deze deed dat eerst ook niet en meldde toch PASS — Codex ronde 3, blocker `geoxyz-branch` F01. `--self-test` breekt daarom één vertaling op een synthetische kopie van GEOxyz en eist dat het script erop faalt; draai die als je aan dit script sleutelt. Toegestane afwijkingen staan met hun reden in `docs/features/<slug>/symmetry-allow.txt`. Ontstaan uit Codex ronde 2, blocker `geoxyz-branch` F02 |
| `tools/dev-server.sh`, `tools/dev-seed.rb`, `tools/verify-lib.mjs` | G9. `dev-server.sh` stopt nu als het laden van de standaarddata of de seed faalt, in plaats van een server op een lege database te presenteren; `go()` in `verify-lib.mjs` kijkt naar de HTTP-status en niet naar de paginatitel, zodat een 403 geen bewijsmateriaal meer wordt |
| `tools/test-env.sh <worktree> <cmd>` | `test:all` bruikbaar maken (zonder dit ~260 systeemtestfouten). Weigert nu als er geen Chromium is, in plaats van die 260 fouten te laten ontstaan en als suitecijfer te laten noteren |
| `tools/inv4-identity.sh` | gedeeld, wordt alleen gesourced: het ene INV-4-patroon plus `inv4_identities <van> <naar>`, die weigert als de range niet oplost |

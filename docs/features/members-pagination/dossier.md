...
| "A `pluck` of every member id still touches every row." | It does, and it is worth being exact about it: `Member.sorted` joins roles, so for *N* members holding *R* roles each the `pluck` returns *N × R* rows and `uniq` reduces them to *N* in Ruby. Page 1 therefore costs the same as page 200. What it replaces is `memberships.preload(:project).sorted.to_a`, which built *N* `Member` objects with their principals and roles; trading *N* objects for *N × R* integers is the difference between a few milliseconds and the seconds #43355 reports. |
| "Then why not do the `LIMIT`/`OFFSET` in SQL and touch only one page?" | Because the duplicate role rows have to be collapsed there too, and the portable way to do it costs more than it saves. `GROUP BY members.id` needs every ordering column in the `GROUP BY` as well on PostgreSQL, which is strict about it — that is `users.type`, `users.firstname`, `users.lastname` and `users.id` from `Principal.fields_for_order_statement`, plus `roles.position` wrapped in a `MIN`. `DISTINCT ON` is cleaner and PostgreSQL-only, and Redmine supports MySQL and SQLite too. Plucking the ids is the portable middle, and it is a deliberate choice rather than an oversight. If a *very* large installation ever needs the SQL version, that is a separate change with its own database matrix to prove. |

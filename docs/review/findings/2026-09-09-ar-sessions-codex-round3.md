...
- **Dossier read:** `docs/features/ar-sessions/dossier.md` — no (company-local)
- **Status read:** `docs/features/ar-sessions/status.md` including settled trade-offs — yes
...
The JSON shape guard remains correct and the serializer still excludes Marshal. Core session shapes, malformed rows, security, storage growth and deploy failure modes were rechecked; no new defect.

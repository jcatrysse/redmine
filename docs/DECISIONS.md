...
| 2026-09-01 | INV-5 verruimd: `en.yml` **plus** `nl.yml`, geen bulk machinevertalingen | Jans vraag "is dat slim of willen ze liever alle talen mee?" leidde tot nakijken. De officiële Contribute-pagina stelt geen regel over locales; hun historie laat per-taal vertaalcommits van moedertaalsprekers zien. Een vertaling die jij zelf kan verantwoorden is een bijdrage; vijftig die niemand kan nakijken zijn een risico. |
| 2026-09-01 | `fr.yml` voorlopig **niet** meesturen | Alleen talen die Jan persoonlijk kan verantwoorden. Eén regel in `tools/check-patch-clean.sh` als hij dat wel wil. |
